#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "sym_tab.h"

table* init_table()	
{
	/*
        allocate space for table pointer structure eg (t_name)* t
        initialise head variable eg t->head
        return structure
    	*/
    table* tab = (table*)malloc(sizeof(table));
    tab->head = NULL;
    return tab;
}

table* allocate_space_for_table() {
    table* new_table = init_table();
    return new_table;
}

symbol* init_symbol(char* name, int size, int type, int lineno, int scope) //allocates space for items in the list
{
	/*
        allocate space for entry pointer structure eg (s_name)* s
        initialise all struct variables(name, value, type, scope, length, line number)
        return structure
    	*/
    symbol* s = (symbol*)malloc(sizeof(symbol));
    // strcpy(s->name, name);
    s->name = name;
    s->size = size;
    s->type = type;
    s->line = lineno;
    s->scope = scope;
    s->next = NULL;
    s->val="~";

    return s;
}

symbol* allocate_space_for_table_entry(char* name, int size, int type, int lineno, int scope) {
    symbol* entry = init_symbol(name, size, type, lineno, scope);
    return entry;
}

void insert_into_table(char* name, int size, int type, int lineno, int scope)/* 
 arguments can be the structure s_name already allocated before this function call
 or the variables to be sent to allocate_space_for_table_entry for initialisation        
*/
{
    /*
        check if table is empty or not using the struct table pointer
        else traverse to the end of the table and insert the entry
    */
   symbol* sym = allocate_space_for_table_entry(name, size, type, lineno, scope);
   if(tab->head == NULL) {
    tab->head = sym;
   }
   else {
    symbol* cur = tab->head;
    
    while(cur != NULL && cur->next != NULL) {
        cur = cur->next;
    }
    cur->next = sym;
   }
}

int check_symbol_table(char* name) //return a value like integer for checking
{
    /*
        check if table is empty and return a value like 0
        else traverse the table
        if entry is found return a value like 1
        if not return a value like 0
    */
   if(tab->head == NULL) {
    return 0;
   }
   else {
    symbol* entry = tab->head;
    while(entry != NULL) {
        if(strcmp(entry->name, name) == 0) {
            return 1;
        }
        entry = entry->next;
    }
   }
   return 0;
}

void insert_value_to_name(char* name, char* value)
{
    /*
        if value is default value return back
        check if table is empty
        else traverse the table and find the name
        insert value into the entry structure
    */
   symbol* cur = tab->head;
    while(cur != NULL) {
        if(strcmp(cur->name, name) == 0 && cur->val != NULL) {
            cur->val = value;
        }
        cur = cur->next;
    }
}

void display_symbol_table()
{
    /*
        traverse through table and print every entry
        with its struct variables
    */
   symbol* cur = tab->head;

   if(tab->head==NULL){
        printf("table is empty \n");
        return;
    }
    printf("--------------symbol table--------------------\n");
    printf(" name\tsize\ttype\tlineno\tscope\tval \n");
    for(symbol*curr=tab->head;curr!=NULL;curr=curr->next){
        printf(" %s\t\t%d\t\t%d\t\t%d\t\t%d\t\t%s \n",curr->name,curr->size,curr->type,curr->line,curr->scope,curr->val);
    }
    printf("----------------------------------------------\n");
}

// helper functions
symbol* get_symbol_by_name(char* name) {
    if(tab->head == NULL) {
        return NULL;
    }
    else {
        symbol* cur = tab->head;

        while(cur != NULL) {
            if(strcmp(cur->name, name) == 0) {
                return cur;
            }
            cur = cur->next;
        }
    }
}

// int handle_arithmetic_operations(symbol* e, symbol* t, int op) {
//     if(e->type == t->type) {
// 				if(e->scope > 0) {
// 					if(t->scope > 0) {
// 						switch(e->type) {
// 							case INT: int t1 = atoi(e->val);
// 							int t2 = atoi(t->val);
//                             int res;
                            
//                             if(op == ADD)
// 							res = t1 + t2;
                            
//                             if(op == SUB)
//                             res = t1 - t2;
                            
//                             if(op == MUL)
//                             res = t1 * t2;
                            
//                             if(op == DIV)
//                             res = t1 / t2;

// 							char* result[10];
// 							iota(res, result, 10);
// 							free(e->val);
// 							e->val = result;
// 							break;

// 							case CHAR:yyerror("%s + %s, Char additions are not allowed!", e->name, t->name);
// 							break;

// 							case FLOAT: float t1 = atof(e->val);
// 							float t2 = atof(t->val);
//                             float res;

//                             if(op == ADD)
// 							res = t1 + t2;

//                             if(op == SUB)
//                             res = t1 - t2;

//                             if(op == MUL)
//                             res = t1 * t2;

//                             if(op == DIV)
//                             res = t1 / t2;

// 							char* result[13];
// 							gcvt(res, 12, result);
// 							free(e->val);
// 							e->val = result;
// 							break;

// 							case DOUBLE: double t1, t2;
// 							sscanf(e->val, "%lf", &t1);
// 							sscanf(t->val, "%lf", &t2);
//                             double res;

//                             if(op == ADD)
//                             res = t1 + t2;

//                             if(op == SUB)
//                             res = t1 - t2;

//                             if(op == MUL)
//                             res = t1 * t2;

//                             if(op == DIV)
//                             res = t1 / t2;

// 							double res = t1 + t2;
// 							char* result[30];
// 							sprintf(result, "%lf", res);
// 							free(e->val);
// 							e->val = result;
// 							break;
// 						}
// 					}
// 					else {
// 						yyerror("variable %s is out of scope", t->name);
// 					}
// 				}
// 				else {
// 					yyerror("variable %s is out of scope", e->name);
// 				}
// 			}
// 			else {
// 				yyerror("Variables of %s and %s have to be of the same type for the operation", e->name, t->name);
// 			}
// }